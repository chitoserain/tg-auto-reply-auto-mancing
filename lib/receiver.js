...
async function waitForText(client, peer, regex, opts = {}) {
    ...
    let seenTop = opts.sinceId || await latestMessageId(client, peer);
        ...
        for await (const m of client.iterMessages(peer, { limit: 5 })) {
            ...
            if ((m.id || 0) > seenTop) {
                if (opts.onNewMessage) {
                    await opts.onNewMessage(m);
                }

                if (typeof m.message === 'string' && regex.test(m.message)) {
                    return m;
                }
            }
        }
    ...
    while (Date.now() - start < timeoutMs) {
        ...
        for await (const m of client.iterMessages(peer, { limit: 5 })) {
            ...
            if ((m.id || 0) > seenTop) {
                ...
                if (typeof m.message === 'string' && regexes.some((r) => r.test(m.message))) {
                    return m;
                }
            }
        }
